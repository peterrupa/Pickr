...
describe('Create Account', () =>{
    ...
    it('should successfully create an account if all values are present', (done) => {
        createAccount(200, 'Name', 'M', 'Last', randomUser, randomUser
        +'@test.com', 'useruser')
            .end((err,res) => {
                should.exist(res);
    ...
    it('should return an error if the parameters are incomplete', (done) => {
        api.post('/api/account/createAccount')
                .send('fname=' + 'fname' + '&mi=' + 'mi' + '&lname=' + 'lname' + '&username=' + 'username')
                .expect(400)
                    .end((err,res) => {
                        should.exist(res);
                        should.not.exist(err);
...
describe('Login', () => {
    ...
    it('should allow case-insensitive username login', (done) => {
        login(200,'PJhROBleS','ultimatesecret')
            .end((err,res) => {
                should.exist(res);
                should.not.exist(err);
    ...
    it('should fail if a session already exists', (done) => {
        login(200,'PJHRobles','ultimatesecret')
            .end((err,res) => {
                should.not.exist(err);
                login(403,'matigas','matigasnasecret')
                    .end((err,res) => {
                        should.exist(res);
                        should.not.exist(err);
                    });
    ...
    it('should fail if the password is incorrect', (done) => {
        login(403, 'PJHRobles', 'notasecret')
            .end((err,res) => {
                should.exist(res);
                should.not.exist(err);
                done();
